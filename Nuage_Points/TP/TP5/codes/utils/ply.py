...
def write_obj(verts, faces, file_name):
    """Write a mesh to obj file

    Params:
        verts      (np.array): the list of vertices
        faces      (np.array): the list of faces
        file_name  (str)     : the file path
    """
    
    str_verts = [("v " + " ".join(map(str, vert))) for vert in verts]
    str_faces = [("f " + " ".join(map(str, face + 1))) for face in faces] 
